...
Windows 安装时需要确认一次管理员权限。安装程序会自动启动 WebClient、把
WebDAV 单文件上限调整为约 4 GB，并仅将 `10.*`、`172.16-31.*` 和
`192.168.*` 三个私有地址段加入本地 Intranet。卸载时会恢复原文件大小设置，
并删除由握手创建的 Intranet 条目。

## 跨平台共享

1. 两台电脑连接同一个局域网，并分别运行握手。
2. 在共享端添加共享文件夹；也可以设置至少 6 位共享密码进行保护。
3. 系统首次弹出防火墙提示时，允许应用访问“专用网络”。
4. 在另一台电脑点击“刷新”；有密码时输入密码配对，无密码时直接连接。
